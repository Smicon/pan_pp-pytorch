...
parser.add_argument("--thread_num", type=int, default=5, help="server thread")
parser.add_argument("--process_num", type=int, default=1, help="server process, default is 1")
...
class MainHandler(tornado.web.RequestHandler):
    executor = ThreadPoolExecutor(args.thread_num)
    ...
    def call_service(self, img):
        return service.run_v1(img)
    ...
    def post(self, *args, **kwargs):
        ...
        bboxes, mides = yield self.call_service(img)
...
if __name__ == '__main__':
    ...
    app = tornado.web.Application([
        (r"/word-ocr-detect", MainHandler),
        (r"/word-ocr-detect/debug", MainHandlerDebug),

    ])
    http_server = tornado.httpserver.HTTPServer(app)
    http_server.bind(args.port)
    print("bind to port: {}".format(args.port))
    http_server.start(args.process_num)
    print("process_num: {}".format(args.process_num))
    print("thread_num: {}".format(args.thread_num))
    ...
    with torch.no_grad():
        ...
        # 9.232
        # config = r"config/pan_pp/pan_pp_r18_ctw.py"
        # ckpt = r"../checkpoints/test1.1/checkpoint.pth.tar"

        9.2321
        config = r"config/pan_pp/pan_pp_r18_ctw.py"
        ckpt = r"../checkpoints/test0.11/checkpoint.pth.tar"

        # 9.2322
        # config = r"config/pan_pp/pan_pp_r18_ctw.py"
        # ckpt = r"../checkpoints/test0.12/checkpoint.pth.tar"
