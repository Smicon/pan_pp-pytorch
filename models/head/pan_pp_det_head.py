...
class PAN_PP_DetHead(nn.Module):
    ...
    def get_results(self, out, img_meta, cfg):
        ...
        # image size
        org_img_size = img_meta['org_img_size'][0].numpy()
        ...
        for i in range(1, label_num):
            ...
            min_area = cfg.test_cfg.min_area  # derive contours in feature map with scale=1, not 4
            if points.shape[0] < min_area:
                label[ind] = 0
                print("min_area ignored: {} < {}".format(points.shape[0], min_area))
                continue

            score_i = np.mean(score[ind])
            if score_i < cfg.test_cfg.min_score:
                label[ind] = 0
                print("min_score ignored: {} < {}".format(score_i, cfg.test_cfg.min_score))
                continue
