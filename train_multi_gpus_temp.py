...
# os.environ["RANK"] = str(当前进程的排名)
# os.environ["WORLD_SIZE"] = str(总进程数)
# python -m torch.distributed.launch --nproc_per_node={4} train_multi_gpus_temp.py
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', help='config file path', type=str,
                        default="./config/pan_pp/pan_pp_r18_ctw.py")
    # 这里尽量写绝对路径，如果是相对路径，则相对的是工作目录
    parser.add_argument('--checkpoint', nargs='?', type=str, default="./checkpoints/test11")
    parser.add_argument('--resume', nargs='?', type=str, default="./checkpoints/test1/checkpoint.pth.tar")
    parser.add_argument('--local_rank', default=-1, type=int, help='node rank for distributed training')
    parser.add_argument("--nproc_per_node", type=int, default=4, help="Number of GPUs per node")
